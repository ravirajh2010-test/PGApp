...
const Register = () => {
  const [form, setForm] = useState({ name: '', email: '', password: '', orgCode: '' });
  ...
  return (
    <div className="flex min-h-[calc(100vh-8rem)] items-center justify-center bg-gradient-to-br from-slate-50 to-brand-50 px-3 py-4 dark:from-slate-900 dark:to-dark-900">
      <div className="w-full max-w-lg rounded-2xl border border-slate-100 bg-white p-6 shadow-xl dark:border-slate-700 dark:bg-dark-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div className="grid gap-3 sm:grid-cols-2">
            <Input
              label={<FormattedMessage id="auth.orgSlug" defaultMessage="Organization ID" />}
              type="text"
              placeholder="e.g. ORG-000123"
              value={form.orgCode}
              onChange={(e) => setForm({ ...form, orgCode: e.target.value.toUpperCase() })}
              helper="Ask your PG admin for this organization ID"
              required
